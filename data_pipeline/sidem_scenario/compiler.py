...
class ScenarioCompiler(ScenarioFileIO):
    ...
    def _canonical_text_token(cls, value: Any, field_name: str) -> str:
        return text_identity.canonical_text_token(value, field_name, cls.TEXT_TOKEN_PATTERN)

    _canonical_source_file = staticmethod(text_identity.canonical_source_file)

    normalize_source_text = staticmethod(text_identity.normalize_source_text)

    @classmethod
    def source_text_hash(cls, text: Any) -> str:
        return text_identity.source_text_hash(text, normalize=cls.normalize_source_text)
    ...
    _speaker_identity = staticmethod(text_identity.speaker_identity)
    ...
    def _apply_audio_state_command(self, command: str, vals: list):
        effect = apply_audio_state_command(self.state, command, vals, self._safe_float)
        if effect.clear_bgm_inheritance:
            self._bgm_from_advbackground = False
        if effect.clear_ambient_inheritance:
            self._environmental_from_advbackground = False
        if effect.changed:
            self._mark_stage_change()

    def _bgm(self, vals: list):
        self._apply_audio_state_command('bgm', vals)

    def _bgm_stop(self, vals: list):
        self._apply_audio_state_command('bgm_stop', vals)

    def _se(self, vals: list):
        self._apply_audio_state_command('se', vals)

    def _se_stop(self, vals: list):
        self._apply_audio_state_command('se_stop', vals)

    def _environmental(self, vals: list):
        self._apply_audio_state_command('environmental', vals)

    def _environmental_stop(self, vals: list):
        self._apply_audio_state_command('environmental_stop', vals)

    def _environmental_volume(self, vals: list):
        self._apply_audio_state_command('environmental_volume', vals)

    def _environmental_ducking(self, vals: list):
        self._apply_audio_state_command('environmental_ducking', vals)
