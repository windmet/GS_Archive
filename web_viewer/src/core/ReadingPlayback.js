...
export function readingPlaybackTarget(document, rowId, revision, entry) {
  ...
  if (document?.status !== 'ready' || !row || ['title', 'synopsis'].includes(row.kind)) {
    throw Error('这一行暂时不能定位演出，请选择正文台词。')
  }
  const range = row.anchor.playback
  return {
    ...
    async readScenario(response) {
      ...
      const step = scenario.steps?.[row.anchor.step_index]
      if (scenario.steps?.length !== document.source.step_count || step?.step_id !== row.anchor.step_id || step?.type === 'synopsis') {
