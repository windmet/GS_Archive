...
import { AlignLeft, ArrowRight, BookOpen, Play } from '@lucide/vue'
...
.synopsis-band { position: relative; padding: 24px max(190px, calc((100% - 820px) / 2)) 26px max(24px, calc((100% - 1120px) / 2)); border-bottom: 1px solid #dce4e5; background: #eaf6f4; }.section-label { display: flex; align-items: center; gap: 6px; color: #147c75; font-size: .62rem; font-weight: 800; }.synopsis-copy h3 { margin: 11px 0 7px; font-size: .92rem; }.synopsis-copy p, .missing-copy { max-width: 760px; margin: 0; color: #405159; font-size: .72rem; line-height: 1.85; white-space: pre-line; }.missing-copy { margin-top: 10px; color: #758188; }
.synopsis-band > button { position: absolute; top: 50%; right: max(24px, calc((100% - 1120px) / 2)); display: inline-flex; align-items: center; justify-content: center; gap: 7px; min-width: 132px; min-height: 40px; padding: 8px 13px; border: 1px solid #158f87; border-radius: 6px; background: #158f87; color: #fff; cursor: pointer; font: inherit; font-size: .7rem; transform: translateY(-50%); }.synopsis-band > button:disabled { border-color: #cbd3d6; background: #dfe5e7; color: #78848a; cursor: not-allowed; }.synopsis-band > small { position: absolute; right: max(24px, calc((100% - 1120px) / 2)); bottom: 15px; color: #66817e; font-size: .54rem; }
...
@media (max-width: 760px) { .story-identity { grid-template-columns: 1fr; gap: 16px; padding: 15px 12px 18px; }.identity-copy h2 { font-size: 1.05rem; }.synopsis-band { padding: 19px 13px; }.synopsis-band > button { position: static; width: 100%; margin-top: 16px; transform: none; }.synopsis-band > small { position: static; display: block; margin-top: 7px; text-align: center; }.detail-section { padding: 18px 12px; }.episode-list, .related-list { grid-template-columns: 1fr; }.source-strip { flex-direction: column; padding: 13px 12px 18px; }.source-strip code { text-align: left; } }
</style>
