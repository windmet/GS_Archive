...
## 2026-09-13偶像选择页可达性修正

用户在桌面100%缩放下无法滚到名单底部。核对发现外层`.archive-content`裁剪溢出，而旧选择页以`min-height:100%`增长，实际滚动只发生在内层名单；操作区因此可能被截在视口外。选择页现固定为内容区域高度并自身滚动，名单不再套独立滚动区；加入姓名/组合搜索、分页及跨页保持的已选人物提示。桌面按宽度每页12位，700px及以下每页3位，窄屏允许纵向滚动；不再以视口高度把矮桌面误判为窄屏。

5175实际Browser核对：2048×1105为12位且操作区同屏；1280×720及1365×600仍为12位，滚到底均能操作；390×844、320×740、390×542各为3位，底部按钮可达，无横向溢出或console error。五页覆盖全部49位且无重复，姓名搜索与选择可用；完整Welcome的390px入口在选择偶像后也可滚到Portal按钮。`npm run build:check`通过，未复制public资源。本修正仅验收选择页交互，不扩展为媒体加载或全站UX重新验收。
