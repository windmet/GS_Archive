...
> **新窗口接续规则（核对代码 bce13c3）**：请先读 [新窗口唯一交接](SIDEM_FRESH_WINDOW_HANDOFF_20260913.md)。本文件保留历史记录；其中旧“下一步”不再自动执行。新窗口按G0复核→F1个人统计readiness→F2代表色头像分批收口，然后停止等待用户选择主线；不再派发前几个长上下文窗口，不自动扩展P3/P4/S1–S4。Welcome三人分组已被49人自然滚动取代，禁止按旧记录复活。

> 2026-09-13 下一步主方向：见 [实体展示、导航Chrome与Song时间轴审计路线](PRESENTATION_CHROME_SONG_ROADMAP_20260913.md)。已核对轻启动后的代码与本地索引，吸收三份审阅及drv999社长愚人节单人小人澄清。本次是路线文档，未实施新展示/歌词/舞台功能；先接收人物选择修复，再分批处理Cards身份、跨域引用、返回语义与Chrome，随后轻量timeline、时钟/歌词及按需舞台。
...
## SUPERSEDED：Welcome偶像选择每组最多三位（已被f8b0b30取代）
...
## Welcome偶像选择：取消三人分组上限

输入HEAD `e7ace21`。用户进一步指出三人分组使桌面90%缩放时仍只显示三人，并明确窄屏允许向下滚动。现取消上一组/下一组/快速跳组和每组三人的截断，49位连续显示；桌面每行3人、平板2人、窄屏1人，整页纵向滚动。保留搜索、随机选择与已选人物；随机和“查看已选”会滚动并聚焦到对应卡片。非首屏头像延迟加载。

5175 IAB实测完整名单、北斗搜索及随机选择，console error/warn 0。Edge在1365×768、900×768、390×844分别核对3/2/1列；首屏完整可见卡片分别为15/10/5张，49张可滚动到末张，底部操作按钮可到达，横向溢出和页面脚本错误均为0。桌面还验证末张选中后“查看已选”能定位并聚焦、北斗搜索仅剩1张、空搜索有提示。`verify:archive-startup-route`及`build:check`通过，代码产物仅在E盘`.analysis/build-check`，未复制public。先前三人分组记录保留为历史实现，不再代表当前行为。

## P3 部分实施：后续步骤慢资源时保留上一帧

输入HEAD `f8b0b30`。原Player在后续步骤等待新人物时，App全屏LoadingScreen遮住正在播放的画面，SpineStage又先卸下旧人物。现由导航在改写当前步索引前捕获已绘制的Pixi帧；目标人物或背景仍待资源时，局部遮罩保留旧场景，直接ADV切换还保留旧台词，text_disable桥段则只保留舞台像素、不重新显示被剧本隐藏的台词。当前步报告等待时，若旧帧可用便隐藏全屏LoadingScreen并显示局部“正在准备下一段画面…”；目标步playable后先绘制目标舞台，再释放旧帧、解除buffering暂停。退出播放器时释放画布，保留既有重试/返回机制。截图提取只针对需换人物/背景的后续步触发，留存画布至多约207万像素；1280×900无头Edge提取/渲染实测约12–68ms，这不是用户电脑的性能上界。
...
输入HEAD `3fd055e`。此前运行时把新步骤标记为playable后才开始获取/解码语音；受控延迟活动430018第二话的`1_3_30018_01_c1002.m4a`时，8/26台词已经出现，声音却仍未启动。现在每个运行时代次在背景就绪后先准备本步语音，成功才启动cue并发布playable；跳步或退出会取消旧请求，迟到结果不得改写新步骤。后续未缓存语音会在旧帧上显示“正在准备下一段语音…”，404则进入可重试的`voice-renderable`阻断。已准备的语音直接交给播放阶段消费，避免同一步二次请求；恢复历史步骤保持不自动重播语音。

`verify:story-loading-safety`、`verify:story-runtime-foundation`、`verify:story-step-playback-state`、`verify:story-audio`及`build:check`通过；代码产物仍仅在E盘固定`.analysis/build-check`，不复制public。5175受控Browser在1280宽与390×844延迟上述GET时分别确认旧帧保留、局部等待提示、放行后8/26开始语音且遮罩释放、无横向溢出和页面脚本错误；404后恢复资源并点“重试当前段落”可播放。等待期间点“上一段”后再放行旧GET，旧语音未启动，页面脚本错误0。IAB正常同话可到5/26、console error 0。

这仍不是P3完成：逻辑步索引仍先改写，Spine实例/纹理的真正提交前准备、复杂跳转/暂停矩阵、长音频听感和媒体发布包均未验收。当前旧帧捕获以解码PCM是否存在判断；PCM命中但口型JSON仍迟到的情况未覆盖，不能据本批声称所有语音相关等待都保留旧帧。用户要求本批收尾后暂停，后续从该边界继续。
