...
两种效果管理器共用 effectTextureCache，缓存成功/进行中的 Promise，失败时
仅删除仍指向自身的条目，再抛出原错误。失败不再永久污染同名效果，旧请求
不能删除新缓存。舞台图片加载器增加 allowFallback（默认 true），特效消费者
显式 false，使网络失败或无效纹理超时可进入原有雨层降级/效果错误处理，而非
把占位纹理当成功永久缓存；普通背景/角色加载保留原默认行为。

screen-clock 纳入两种真实管理器的并发、失败重试、成功复用、同步异常和
旧失败隔离测试；另用受控 Image.onerror 验证舞台严格失败与默认占位分支。
