...
后续 [64k 台词语音替换](VOICE64_REPLACEMENT_20260924.md)已完成，32,421 条语音全量远端 checksum 及 160 条 HTTP 抽查通过，当前桶总量 **6,574,047,523 B**。当前完整发布清单为 `.deploy/voice64/full-manifest.json`。下段的 7.831 GB 为 gzip 整桶迁移完成时的历史值。

用户已将六批迁移改为整桶清空后完整重传，当前执行依据为 [整桶重传记录](STORAGE_FULL_REUPLOAD_20260924.md)。98,103 个旧对象已删除并确认空桶；98,032 个新对象已完整上传并通过统一 checksum、键集合及大小校验。实测总量 7,830,976,511 B，含其他用途最低预留的账户预算为 8,710,976,511 B。全量模式及固定数据 revision 已部署到 [新预览](https://3412e295.gs-archive-preview.pages.dev)，gzip 与媒体 HTTP 验证通过；浏览器播放专项验收仍待完成。

下文保留各阶段历史记录。早期 canary 的双语缺失已通过翻译打包修复，但这些历史结果不能代替整桶重传后的 HTTP/Browser 验收；云端口型专项观察仍未完成。语音 64k 已由用户确认听感可接受并明确授权替换，完成情况见上方记录。

## 范围和默认状态

本轮从 `origin/master` 产品基线 `3e547e7` 新开 `codex/storage-compression`，只 cherry-pick 两个审计文档提交，不包含 About UI。

初次本地实现为 **默认关闭的 gzip-v1 增量部署通道**。现有 PNG/WebP、音频、歌曲、背景和 v2 完整 exporter 保持原行为。原 `.deploy/r2`、`.deploy/r2-manifest.json` 不覆盖、不裁剪。该阶段尚未进行远端上传、配置切换或 GC；后续执行以本文最新状态和整桶重传记录为准。
