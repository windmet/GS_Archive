...
起点 `1d52a25`。检查发现第二批移除 `entry['3']` 的直接显示时，也隐藏了表 43
中实际存在的正式剧情标题，导致 313 条关联记录误显示“剧情标题待确认”。
数据仍完整保留，问题在展示层，不应再向 Wiki 重复索取本地已有标题。

新增 CardPresentation 的 cardScenarioTitle，优先使用现有 display_title，只有
已确认来自表 43 时才读取旧字段 3。表 32 的字段 3 是内部标签，不作为默认标题；
未知来源、空值或与资源身份相同的值继续回退。compiled_summary.title 可能是
首句对白，不借用它代替标题。原字段和导航 payload 保持原样。

展示门禁核对全部 425 条卡片剧情关联记录的标题保留、表 32 标签排除、来源
不明回退和输入不变性；verify:card-voice-preview 与 source-only 构建通过。
Browser 实测「瞳に映るその先に」恢复「冷めない熱」「気持ちに応えたい」，
「スカウト後トーク」仍保留规范展示名；电话剧情载入并返回正常，无应用 error。
教训：禁止技术身份泄露不等于禁止读取所有旧字段，必须按字段语义做展示投影。
