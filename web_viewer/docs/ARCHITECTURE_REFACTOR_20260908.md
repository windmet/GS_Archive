...
| G，推进中 | G1 建立 sidem_scenario 包，分离 state/compiler/file_io/cli，旧入口重新导出同一类；G2 注入资源接口并隔离候选任务缓存；G3 统一候选来源配置；G4 分离 RAW transport/identity/voice 规则；G5 分离 masterdata wire，见 `SCENARIO_PIPELINE_BOUNDARIES.md` | 10 组旧编译器输出/provenance hash、导入/CLI/临时批处理与 RAW→Spine 集成通过；资源 provider 与候选输出回归通过；候选配置与 JS 根路径一致性通过；RAW 单 bundle 旧新逐值一致；masterdata 47,204 记录旧新 hash 一致；旧 API 默认、领域生成/publish 分层仍待迁移 |
