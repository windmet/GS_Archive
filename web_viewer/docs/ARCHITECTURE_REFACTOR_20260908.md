...
| G，推进中 | G1 建立 sidem_scenario 包，分离 state/compiler/file_io/cli，旧入口重新导出同一类；G2 注入资源接口并隔离候选任务缓存，见 `SCENARIO_PIPELINE_BOUNDARIES.md` | 10 组旧编译器输出/provenance hash、导入/CLI/临时批处理与 RAW→Spine 集成通过；资源 provider 与候选输出回归通过；配置统一、RAW/masterdata/publish 分层仍待迁移 |
