...
| B，推进中 | 剧情语义与状态 ownership characterization；B1 拆 Spine cue 执行并修复取消等待，B2 补异步模型发布的场景归属，见 `STORY_STATE_OWNERSHIP.md` | RAW fixture → 两种编译路径 → Spine adapter 已接通；模型替换/离场竞态回归及三角色浏览器冒烟通过；metadata、其余舞台属性时序与跨 channel 状态仍需推进 |
