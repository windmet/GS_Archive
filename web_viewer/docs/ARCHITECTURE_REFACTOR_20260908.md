...
| G，推进中 | G1 建立 sidem_scenario 包，分离 state/compiler/file_io/cli，旧入口重新导出同一类，见 `SCENARIO_PIPELINE_BOUNDARIES.md` | 10 组旧编译器输出/provenance hash、导入/CLI/临时批处理与 RAW→Spine 集成通过；资源依赖、RAW/masterdata/publish 分层仍待迁移 |
