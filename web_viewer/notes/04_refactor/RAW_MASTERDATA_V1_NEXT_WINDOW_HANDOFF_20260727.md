...
物理 USM 默认来自 `RAW/movie`；119 份 `liveeffectscript` CSV 仍明确属于
`legacy_root` 下的语义参考。FFmpeg、FFprobe 和 WannaCRI 根目录由忽略的
...
| 已有统一 archive root 配置 | 现在正确 | Python/JS loader、RAW 审计器、Vite、live audio 与 Backmonitor 已接入；其余五个 chibi 辅助脚本仍需逐个迁移 |
...
以及 live-chibi 音频与 Backmonitor 构建器已经接入。新窗口默认继续其余仍硬编码旧整理包
...
当前仍有五个明确目标，建议按“单一 RAW 域优先、外部容器最后”的顺序：

1. `prepare-live-chibi-image-layers.py`：`RAW/asset` + legacy liveeffectscript；
2. `prepare-live-chibi-object-layers.py`：`RAW/asset` + legacy liveeffectscript；
3. `prepare-live-chibi-stage-backgrounds.py`：`RAW/asset` + legacy liveeffectscript；
4. `prepare-live-chibi-assets.py`：RAW、costume、live lipsync 等混合来源；
5. `prepare-live-chibi-stage-effects.py`：XAPK/APK 外部容器，单独定义来源字段。

下一批首选 `prepare-live-chibi-image-layers.py`。先确认其 57 个已发布 PNG
对应的 RAW bundle/Sprite 集合，再增加 `--sources-config`、显式输入/输出
覆盖和隔离候选验证；不要在同一提交顺带改 object/stage-backgrounds。
