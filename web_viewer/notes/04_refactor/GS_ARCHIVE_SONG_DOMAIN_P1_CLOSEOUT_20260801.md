...
The first experiment uses `drvalv` with `001tom` (天ヶ瀬 冬馬) and prepares only
the requested personal vocal ACB plus `song3_drvalv_bgm.acb` as browser-readable
M4A candidates. Existing local Chibi music candidates provide the single-track
original, unit, and `drv999` special-version choices. The song detail page now
...
is generated by `scripts/prepare-song-experimental-audio.py` and remains under
the existing ignored binary-asset boundary. Use
...
but no audio-mixer parameters. Therefore the next audit target is the client
runtime/CRI cue metadata: look for gain, pan, start offset, fade, loop, or
ducking instructions before treating two-track playback as an official mix.
